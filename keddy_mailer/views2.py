...
import xlrd
from django.conf import settings
from .models import Campaign, SMTPServer, email_list, template,CustomUser,UnsubscribedEmail
from .test_smtp import send_bulk_emails_task
...
#         obj = CustomUser(name = name , email = email , number = number , password = password , dob = dob , address = address,company = company)
...
#         # print(email)
#         # print(password)
#         obj = CustomUser.objects.filter(email = email , password = password,is_active = 1)
#         if obj:
#             print("Welcome User")
#             request.session["email"]= email
#             request.session["name"] = obj[0].name
#             request.session["user_id"] = obj[0].uid
...
#     # 🟢 Click rate (monthly)
#     click_rate = (monthly_clicked / monthly_sent * 100) if monthly_sent else 0
...
#     }
#     # Line Chart Data: Daily email sent counts this month
#     daily_data = (
#         EmailTracking.objects
#         .filter(campaign__in=user_campaigns, sent_at__gte=start_of_month)
...
#     # Prepare labels and data lists

#     data_points = [entry['sent_count'] for entry in daily_data]
#     labels = ['2025-06-03', '2025-06-11', '2025-06-30', '2025-07-06', '2025-07-15', '2025-07-16']
#     campaigns_data = [1, 2, 3, 4, 5, 6]
#     emails_data = [10, 20, 30, 40, 50, 60]
#     clicks_data = [2, 4, 6, 8, 10, 12]
...
#         "daily_data": json.dumps(data_points),
...
def create_campaign(request):
    ...
    if request.method == 'POST':
        ...
        placeholder = request.POST.get('placeholder_text')
        ...
        if user.used_email_credit >= user.email_credit_limit:
            return render(request,"create_campaign.html",{"msg1":"Email credit limit reached"})
        else:
    ...
    context = {
        ...
        'templates': template.objects.filter(user_id=user_id).order_by('-uploaded_at'),
        'smtps': SMTPServer.objects.filter(user_id=user_id).order_by('-created_at'),
        'uname': user.name,
        "company_name":user.company,
        "user":user,
        "credit_limit":credit_limit,
...
def extract_recipients_from_text(text):
    # Split by commas, newlines, and strip spaces
    if not text:
        return []
...
def track_attachment(request, campaign_id, email):
    ...
    # ✅ Decode file name (safety)
    file_name = unquote(file_name)

    # ✅ Update tracking
    updated = EmailTracking.objects.filter(
    ...
    print(f"📎 Attachment tracked for {email} | campaign {campaign_id} | updated: {updated}")

    # ✅ Path where attachments are saved (adjust if different)
...
def adminhome(request):
    ...
    # # Monthly aggregation
    # user_data = CustomUser.objects.annotate(month=TruncMonth("created_at")) \
    #     .values("month").annotate(total=Count("id")).order_by("month")

    # campaign_data = Campaign.objects.annotate(month=TruncMonth("created_at")) \
    #     .values("month").annotate(total=Count("id")).order_by("month")

    # smtp_data = SMTPServer.objects.annotate(month=TruncMonth("created_at")) \
    #     .values("month").annotate(total=Count("id")).order_by("month")

    # # Extracting aligned labels
    # all_months = sorted(
    #     set(entry["month"].strftime("%b %Y") for entry in user_data) |
    #     set(entry["month"].strftime("%b %Y") for entry in campaign_data) |
    #     set(entry["month"].strftime("%b %Y") for entry in smtp_data)
    # )

    # def get_monthly_series(data, all_months):
    #     data_dict = {d["month"].strftime("%b %Y"): d["total"] for d in data}
    #     return [data_dict.get(month, 0) for month in all_months]
...
#     for user in users:
#         used_credit = EmailTracking.objects.filter(campaign__user_id=user.uid).count()
#         credit_left = max(user.email_credit_limit - used_credit, 0)
...
#     return render(request, "user_credits_admin.html", {"users": user_stats})

# def edit_user_credit(request, user_id):
#     user = get_object_or_404(CustomUser, id=user_id)
#     if request.method == 'POST':
#         credit = request.POST.get('email_credit_limit')
#         if credit.isdigit():
#             user.email_credit_limit = int(credit)
#             user.save()
...
#     return render(request, "edit_user_credit.html", {"user": user})
